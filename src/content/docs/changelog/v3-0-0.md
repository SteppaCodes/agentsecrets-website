...
* *Read more in [Secret Policies & Approvals](/docs/concepts/secret-policies).*
...
### 1. Migrate Legacy Configuration
:::step
...
**Verify migration:**
Run the status command to confirm your login state is successfully migrated:
```bash
agentsecrets status
```
:::

### 2. Re-Authorize the Binary Hash (Linux & WSL only)
:::step
...
If you prefer to authorize it manually beforehand, you can run:
```bash
sudo keychain-auth authorize $(which agentsecrets) AgentSecrets
```
:::

### 3. Review Capabilities & Secret Policies
:::step
